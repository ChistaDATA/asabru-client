...
enum T_PARAM_TYPE { ILLEGAL_PARAM = -1, T_CHAR, T_INT, T_FLOAT, T_DOUBLE, T_BUFFER, T_VOID_PTR, T_CHAR_PTR };
...
T_PARAM_LIST *handle_command_upload(T_PARAM_LIST *param_list) {
	...
	return reinterpret_cast<T_PARAM_LIST *>(UploadFile(file_name, server_name, port));
}
...
T_PARAM_LIST *handle_command_async(T_PARAM_LIST *param_list) {
	fprintf(stdout, "%s\n", "ASYNC TASK ..");
	ASYNC_TASK tsk;
	strcpy(tsk.action, to_string(rand()%10000).c_str());
	addAsyncTask(tsk);
	return 0;
}
...
std::map<const std::string, HANDLE_COMMAND> actions = {
	{"show", handle_command_show}, {"list", handle_command_list},	  {"pwd", handle_command_pwd},
	{"ls", handle_command_ls},	   {"upload", handle_command_upload}, {"exit", handle_command_exit},
	{"async", handle_command_async}
	};
